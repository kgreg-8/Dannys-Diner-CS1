...
WHERE row_num = 1;
-- A: sushi | B: curry | C: ramen Key: defining window function in CTE
...
FROM cte
	...
	menu m ON cte.product_id = m.product_id
WHERE row_num = 1;
-- Customer A ordered sushi first after becoming a member | B: curry
...
/*
    ...
    ----------------
*/
-- BONUS 1: Recreate the following table output (columns: customer_id, order_date, product_name, price, member) (member shows if a customer is a member at the time of the order_date in a binary manner)
SELECT
	s.customer_id,
	s.order_date,
	m.product_name,
...
-- BONUS 2: Rank the order of purchases for members (using data table created in bonus 1)
WITH cte AS (
SELECT 
	s.customer_id,
	s.order_date,
...
FROM cte
	LEFT JOIN (
		...
		FROM cte
        WHERE member = 'Y'
		) r ON cte.customer_id = r.customer_id AND cte.order_date = r.order_date
ORDER BY customer_id, order_date;
# For some reason there are 2 orders for customer A that are duplicated. Otherwise this is correct. I think it's a bug (a select distinct added in the cte won't help as the duplicated rows both have the same values for customer A & occur on 2021-01-11.
